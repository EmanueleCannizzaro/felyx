...
int LcmDofSet::GetDofPos( unsigned bitcount ){
...
size_t LcmDofSet::count(unsigned pos){
...
LcmDofSet fe_base::operator& (const LcmDofSet& ds1, const LcmDofSet& ds2){
  LcmDofSet result(ds1);
  result &= ds2;
  return result;
};

//Check if Element is a 3D Solid
bool LcmDofSet::IsElement3DSolid(){
  return ( test(3) && count()==3 );
}

//Check if Element is a 3D Shell
bool LcmDofSet::IsElement3DShell(){
  return ( count()==6 );
}

//Check the dimension of the element
unsigned LcmDofSet::ElementDimension() const {
