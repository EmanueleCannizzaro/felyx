...
int DofSet::GetDofPos( unsigned bitcount ){
...
size_t DofSet::count(unsigned pos){
...
DofSet fe_base::operator& (const DofSet& ds1, const DofSet& ds2){
  DofSet result(ds1);
  result &= ds2;
  return result;
};
...
bool DofSet::IsElement3DSolid(){
  return ( test(3) && count()==3 );
}

//!Check if Element is a 3D Shell
bool DofSet::IsElement3DShell(){
  return ( count()==6 );
}

//Check the dimension of the element
unsigned DofSet::ElementDimension() const {
