...
#ifndef ITL_LDL_SHIFT_ICT_H
#define ITL_LDL_SHIFT_ICT_H
...
namespace itl {
  ...
  */
  ...
  public:
    ...
    typedef preconditioner3 < Matrix1, Matrix2, mtl::unit_lower, mtl::unit_upper> Precond;
    typedef preconditioner1< Matrix1, Matrix2, mtl::unit_lower, mtl::unit_upper> Left;
    typedef preconditioner2< Matrix1, Matrix2, mtl::unit_lower, mtl::unit_upper> Right;
  
    ldl_shift_ict(const SymMatrix& A, int lfil=10, T tol=1e-6, T& shift = 0) 
      : Tri_ptr(A.nrows()+1), 
      diag(A.nrows()),
      w(A.nrows()),
      jw(A.nrows()),
      jr(A.nrows())
    {
      ...
      do_cholesky(A, Orien(), lfil, tol, shift); 
    }
  private:
 
    void 
    do_cholesky(const SymMatrix& A, mtl::row_tag, int lfil, T tol, T& shift)
    {
      using std::copy;
      if ( A.is_upper() ) {
        ...
        for (j = A.begin(); mtl::not_at(j,A.end()); j++) {
          ...
          wnum=0;
          i = j_row.begin();
          *diag_it = *i +shift - *diag_it;
          if (*diag_it < 0) {
            std::cout << "WARNING Negative diagonal element calculated: "  << *diag_it << std::endl;
            T m, new_update;
            m = (neg_diag - *diag_it) / update;
            cout << "m: " << m << endl;
            if (m < 0) { 
              new_update = neg_diag/m;
              cout << "new_update: " << new_update << endl;
              if (new_update > 0) {
                update = new_update;
              }
            }
            shift += update;
            neg_diag = *diag_it;
            std::cout << "Restart! With adapted shift value: " << shift << std::endl;
            diag_it = diag.begin();
            j = A.begin();
            std::fill(jj_index.begin(), jj_index.end(), 0);
            std::fill(jj_pos.begin(), jj_pos.end(), 0);
    ...
    void 
    do_cholesky(const SymMatrix& A, mtl::column_tag, int lfil, T tol, T& shift)
    {
      using std::copy;
      if ( A.is_upper() ) {
