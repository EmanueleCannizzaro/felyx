...
#ifndef ITL_LDL_ROBUST_ICT_H
#define ITL_LDL_ROBUST_ICT_H
...
namespace itl {
  ...
  */
  ...
  public:
    ...
    ldl_robust_ict(const SymMatrix& A, int lfil=10, T tol=1e-8) 
      : Tri_ptr(A.nrows()+1), 
      diag(A.nrows()),
      w(A.nrows()),
      jw(A.nrows()),
  ...
  private:
      ...
      if ( A.is_upper() ) {
        ...
        for (j = A.begin(); mtl::not_at(j,A.end()); j++) {
                ...
                if (*jr_it) {
                  ...
                  if (ABS(Tri_val[l]) > c) {
                    ...
                    *jr_it = wnum;
                    ...
                    //stabilized cancellation
                    diag[Tri_ind[l]] -= ABS(Tri_val[l]*z);
          ...
          *jj_index_it = *jw.begin();
          ...
          //stabilized cancellation
          jw_end = jw.begin()+wnum;
          for (;jw_it != jw_end; ++jw_it) {
            //these elements are beeing dropped
            diag[*jw_it] -= ABS(w[jr[*jw_it] -1]);
          }

          //this should not happen now, since this factoriszation should be robust
          if (*diag_it < 0) {
    ...
    void do_cholesky(const SymMatrix& A, mtl::column_tag, int lfil, T tol) 
      {
      using std::copy;
      if ( A.is_upper() ) {
