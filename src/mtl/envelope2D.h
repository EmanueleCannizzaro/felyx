...
public:
  ...
  public:
    ...
    // --------- start modification by okoenig ------------------
    // Column index j=ii of matrix access has to be passed to elt_ref
    // as a relative index; meaning index 0 points to first stored value in a certain row
  ...
  */
  ...
  void initialize_nzstruct(const Array& a, size_type nnz) {
    val.resize(nnz, T(0));
    for (unsigned x = 1; x < dim.first(); ++x){

      // Replacement by okoenig...
      // Original: ptr[x] = ptr[x-1] + a[x-1];
      ptr[x] = ptr[x-1] + a[x];
    }
