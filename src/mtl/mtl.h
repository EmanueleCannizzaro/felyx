...
{
  MTL_ASSERT( (unsigned) A.nrows() <= (unsigned) zz.size(), "mtl::mult()");
  MTL_ASSERT( (unsigned) A.ncols() <= (unsigned) xx.size(), "mtl::mult()");
  typedef typename matrix_traits<Matrix>::value_type T;
...
template <class TriMatrix, class VecX>
inline void
tri_solve_unit__(const TriMatrix& T, VecX& x, column_tag)
{
  ...
  if (T.is_upper()) {
    typename TriMatrix::const_reverse_iterator T_j; 
    typename TriMatrix::Column::const_reverse_iterator T_ji, T_jrend;
    typename VecX::reverse_iterator x_it = x.rbegin();
    ...
    for (T_j = T.rbegin(); T_j != T.rend(); ++T_j) {
      ...
      for (T_ji = (*T_j).rbegin();T_ji != T_jrend; ++T_ji) {
	Int i = T_ji.row();
	x[i] -= x_j * *T_ji;
      }
      ++x_it;
    ...
    typename TriMatrix::const_iterator T_j; 
    typename TriMatrix::Column::const_iterator T_ji, T_jend;
    typename VecX::iterator x_it = x.begin();
    ...
    for (T_j = T.begin(); T_j != T.end(); ++T_j) {
      ...
      for (T_ji = (*T_j).begin();T_ji != T_jend;++T_ji) {
	Int i = T_ji.index();
	x[i] -= x_j * *T_ji;
      }
      ++x_it;   
...
template <class TriMatrix, class VecX>
inline void
tri_solve_unit__(const TriMatrix& T, VecX& x, row_tag)
{
  ...
  if (T.is_upper()) {
    ...
    for (T_i = T.rbegin(); T_i != T_iend; ++T_i) {
      *x_it -=mtl::dot(*T_i, x);
      ++x_it;
    }
  } else { // T is lower
...
{
  VecX& x = const_cast<VecX&>(x_);
  MTL_ASSERT((unsigned) T.nrows() <= x.size(), "mtl::tri_solve()");
  MTL_ASSERT((unsigned) T.ncols() <= x.size(), "mtl::tri_solve()");
...
template <class TriMatrix, class VecX>
inline void
tri_solve_unit(const TriMatrix& T, MTL_OUT(VecX) x_) MTL_THROW_ASSERTION
{
  VecX& x = const_cast<VecX&>(x_);
  MTL_ASSERT((unsigned) T.nrows() <= x.size(), "mtl::tri_solve()");
  MTL_ASSERT((unsigned) T.ncols() <= x.size(), "mtl::tri_solve()");
...
{
  // cout << "VecX.Size()=" << x.size() << " VecY.size()= " << y.size() << endl;
