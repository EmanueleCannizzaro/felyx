...
public:
  ...
  inline void filter(const T& val)
  {
    iterator iter = begin();
    iterator iter_end = end();
    values_t my_values;
    indices_t my_indices;
    
    while(iter != iter_end) {
    	
    	if (((*iter)>=0 ? (*iter) : -(*iter)) > val) {
	  my_values.push_back(*iter);
    	  my_indices.push_back(iter.index());
	}
	iter++;
    }
    	
    values->swap(my_values);
    indices->swap(my_indices);
  }
