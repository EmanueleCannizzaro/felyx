...
namespace fe_base{		// Put classes into namespace fe_base
  ...
  //! Class DofSet is derived from bitset<6> to handle all dof stuff
  //! in nodes and elements
  class DofSet : public bitset<6>{
    
  public:

    // Constructors
    // ------------
    DofSet() : bitset<6>() {}			 // Empty constructor
    DofSet(const string str) : bitset<6>(str,0,6) {} // "string" constructor
  ...
  // ---------------------------------------------
  DofSet operator& (const DofSet&, const DofSet&);
